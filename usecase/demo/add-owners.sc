...
void main() {
  ...
  pd_shared3p xor_uint32[[1]] key = {
    ...
    0x770A8A65, 0xDA156D24, 0xEE2A0932, 0x77530142};

  // Insert a couple of data owners into database:
  for (uint i = 0; i < 2; ++i) {
    print("Preparing database statement for owner ", i+1);
    tdbVmapClear(params);

    pd_shared3p uint8 ownerId = (uint8) i+1;
    tdbVmapAddValue(params, "values", ownerId);

    // For testing, use the same key for everybody:
    tdbVmapAddValue(params, "values", key[4*i]);
    tdbVmapAddValue(params, "values", key[4*i+1]);
    tdbVmapAddValue(params, "values", key[4*i+2]);
    tdbVmapAddValue(params, "values", key[4*i+3]);
